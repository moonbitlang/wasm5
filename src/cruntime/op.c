...
// OpFn signature: hot fields (pc, sp, fp) passed as integers for register allocation
...
// Helper macros for pointer access from integer registers
#define PC ((uint64_t*)pc)
#define SP ((uint64_t*)sp)
#define FP ((uint64_t*)fp)
...
// pc, sp, fp passed as integers for register allocation
static int run(CRuntime* crt, uint64_t pc, uint64_t sp, uint64_t fp) {
    OpFn first = (OpFn)*(uint64_t*)pc;
    pc += 8;
...
int execute(uint64_t* code, int entry, int num_locals, uint64_t* args, int num_args, uint64_t* result_out, int num_results, uint64_t* globals, uint8_t* mem, int mem_size, int* memory_pages, int* table, int table_size, int* func_entries, int* func_num_locals, int num_funcs, int num_imported_funcs) {
    ...
    // Convert hot state to integers for register allocation
    uint64_t pc = (uint64_t)(code + entry);
    uint64_t fp = (uint64_t)stack;
    uint64_t sp = (uint64_t)(stack + num_locals);
...
int op_call(CRuntime* crt, uint64_t pc, uint64_t sp, uint64_t fp) {
    int callee_pc = (int)*PC; pc += 8;
    int frame_offset = (int)*PC; pc += 8;
    ...
    uint64_t new_fp = fp + frame_offset * 8;
    uint64_t new_pc = (uint64_t)(crt->code + callee_pc);
...
int op_entry(CRuntime* crt, uint64_t pc, uint64_t sp, uint64_t fp) {
    int num_locals = (int)*PC; pc += 8;
    int first_local = (int)*PC; pc += 8;
    int num_to_zero = (int)*PC; pc += 8;
...
int op_copy_slot(CRuntime* crt, uint64_t pc, uint64_t sp, uint64_t fp) {
    int src_slot = (int)*PC; pc += 8;
    int dst_slot = (int)*PC; pc += 8;
...
int op_br(CRuntime* crt, uint64_t pc, uint64_t sp, uint64_t fp) {
    ...
    pc = (uint64_t)(crt->code + target_idx);
    NEXT();
}
DEFINE_OP(br)
...
int op_br_if(CRuntime* crt, uint64_t pc, uint64_t sp, uint64_t fp) {
    int taken_idx = (int)*PC; pc += 8;
    int not_taken_idx = (int)*PC; pc += 8;
    sp -= 8;
    ...
    pc = (uint64_t)(crt->code + (cond ? taken_idx : not_taken_idx));
    NEXT();
}
DEFINE_OP(br_if)
...
int op_wasm_if(CRuntime* crt, uint64_t pc, uint64_t sp, uint64_t fp) {
    int else_idx = (int)*PC; pc += 8;
    sp -= 8;
    int32_t cond = (int32_t)*SP;
    if (!cond) {
        pc = (uint64_t)(crt->code + else_idx);
    }
    NEXT();
}
DEFINE_OP(wasm_if)
...
int op_i64_const(CRuntime* crt, uint64_t pc, uint64_t sp, uint64_t fp) {
    *SP = *PC; pc += 8; sp += 8;  // Push immediate
    NEXT();
}
DEFINE_OP(i64_const)

int op_f32_const(CRuntime* crt, uint64_t pc, uint64_t sp, uint64_t fp) {
    *SP = *PC; pc += 8; sp += 8;  // Push immediate (stored as uint64)
    NEXT();
}
DEFINE_OP(f32_const)
...
int op_memory_size(CRuntime* crt, uint64_t pc, uint64_t sp, uint64_t fp) {
    ...
    *SP = (uint64_t)(uint32_t)pages; sp += 8;
    NEXT();
}
...
int op_i32_load(CRuntime* crt, uint64_t pc, uint64_t sp, uint64_t fp) {
    (void)fp;
    pc += 8;  // Skip align
    uint32_t offset = (uint32_t)*PC; pc += 8;
    pc += 8;  // Skip mem_idx
...
int op_call_indirect(CRuntime* crt, uint64_t pc, uint64_t sp, uint64_t fp) {
    pc += 8;  // Skip type_idx (not needed for simplified impl)
    pc += 8;  // Skip table_idx (assume 0)
    int frame_offset = (int)*PC; pc += 8;
    ...
    uint64_t new_fp = fp + frame_offset * 8;
    uint64_t new_pc = (uint64_t)(crt->code + callee_entry);
